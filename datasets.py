...
class AudioDataset(Dataset):
    def __init__(self, audio_dir, label_json, num_classes=4, sr=16000, channels='mono', window=20, validation_mode=False, margin_ratio=1.0, aug=False):
        ...
        self.pitch_shift_dir = os.path.join(os.path.dirname(audio_dir), "PitchShiftedAudio")

        self.label_map = {"Unknown":0, "창조":1, "아니리":1, "설렁제":2, "경드름":2, "우조":2, "평조":2, "계면조": 3}
        self.loaded_hash = self.get_hash(label_json)
        self.loaded_filename, self.loaded_audio = self.get_audio(audio_dir)
        self.loaded_label, self.label_dict = self.get_label(label_json)
        
        # Pitch shifted audio dictionary - mapping hash_key to list of (shift_value, audio_tensor)
        ...
        for item in tqdm(label_data, desc='Load Label'):
            ...
            for annotation in item["annotations"]:
                for result in annotation["result"]:
                    ...
                    label_ant = result["value"]["labels"][0]
                    start_ms, end_ms = int(round(start*1000)), int(round(end*1000))
                    label_dict[hash_key].append({"start": start_ms, "end": end_ms, "label": label_ant})

                    if label_ant:
                        label[start_ms:end_ms, self.label_map[label_ant]] = 1
                        label[start_ms:end_ms, 0] = 0

            label_dict[hash_key].sort(key=lambda x: x["start"])
    ...
    def get_df(self):
        cnt = 0
        df = pd.DataFrame(columns=['filename', 'start_ms', 'end_ms', 'label'])
        for h in self.loaded_hash:
            filename = self.loaded_filename[h]            
            for d in self.label_dict[h]:
                start, end, label = d['start'], d['end'], d['label']
                df.loc[cnt] = [filename, start, end, label]
                cnt+=1
        df.to_csv('/home/jinin/userdata/Pansori_2025_ISMIR/data/label.csv')

    def load_pitch_shifted_audio(self):
        """Load pitch-shifted audio files for augmentation"""
    ...
    def __getitem__(self, idx):
        ...
        # If augmentation is enabled and we have pitch-shifted versions, 
        # randomly decide whether to use a pitch-shifted version
        if self.aug and hash_key in self.pitch_shifted_audio and random.random() < 0.5:
            # Randomly select one of the pitch-shifted versions
            audio = random.choice(self.pitch_shifted_audio[hash_key])
        else: audio = self.loaded_audio[hash_key]
...
class MelDataset(AudioDataset):
    def __init__(self, audio_dir, label_json, num_classes=4, sr=16000, channels='mono', window=20, n_fft=2048, hop_length=512, target_bins=40, validation_mode=False, aug=False, margin_ratio=1.6):
        super().__init__(audio_dir, label_json, num_classes, sr, channels, window, validation_mode, margin_ratio=margin_ratio, aug=aug)
...
class ChromaDataset(AudioDataset):
    def __init__(self, audio_dir, label_json, num_classes=4, sr=16000, channels='mono', window=20, n_fft=2048, hop_length=512, target_bins=40, validation_mode=False, aug=False, margin_ratio=1.6):
        ...
        self.window_frame = self.window * sr // hop_length
        self.spec_cvt = Spectrogram(n_fft=n_fft, hop_length=hop_length, power=1.0)
        self.spec2chroma = ChromaScale(sample_rate=sr, n_freqs=n_fft//2+1, n_chroma=12)
    ...
    def apply_spec_augmentation(self, chroma):
        """Apply spectrogram augmentations"""
        if not self.aug or random.random() > 0.8: return chroma
        if random.random() < 0.5: chroma = self.apply_spec_pitch_shift(chroma)
        if random.random() < 0.5: chroma = self.freq_mask(chroma)            
        return chroma
    ...
    def __getitem__(self, idx):
        ...
        if self.validation_mode:
            # For validation, get the entire audio and label
            hash_key, filename, audio, label = super().__getitem__(idx)
            chroma, _ = self.get_chroma(audio)
            frame_label = self.ms_to_frame_label(label)
            ...
            # Convert to chroma spectrogram with integrated spectrogram augmentations
            chroma, stretch_factor = self.get_chroma(audio)
            
            # slice chroma to window_frame
            start_idx = (chroma.shape[1] - self.window_frame) // 2
            chroma = chroma[:,start_idx:start_idx+self.window_frame]
            
            # Apply additional chroma spectrogram augmentations
            ...
            frame_label = self.ms_to_frame_label(label)
            frame_label = frame_label[start_idx:start_idx+self.window_frame]
            assert frame_label.shape[0] == chroma.shape[1], f"frame_label.shape[0] != chroma.shape[1]: {frame_label.shape[0]} != {chroma.shape[1]}, time stretch factor: {stretch_factor}, audio length: {audio.shape[1]/self.sr} sec"
            return hash_key, filename, chroma, frame_label