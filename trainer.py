...
class Trainer:
    ...
    # def get_next_train_iter(self, train_iter, train_hash_keys):
        ...
        for fold, (train_idx, test_idx) in enumerate(selection, start=1):
            ...
            while self.global_step < self.num_iterations:
                ...
                try: _, x, y = next(train_iter)
                    ...
                    self.dataset.slice_indices = [si for si in self.dataset.slice_indices if si[0] in train_hash_keys]
                    train_loader = DataLoader(self.dataset, batch_size=self.batch_size, shuffle=True, num_workers=4, pin_memory=True, persistent_workers=True, prefetch_factor=4)
                    train_iter = iter(train_loader) # self.update_train_iter(train_hash_keys)
                    _, x, y = next(train_iter)
                    current_epoch += 1
                ...
                self.global_step += 1
                pbar.update(1)
                pbar.set_description(f"Fold {fold} | Iter {self.global_step}/{self.num_iterations} | Loss: {loss:.4f}, Best Acc : {batch_acc_masked:.4f}")
                ...
                if self.global_step % self.eval_interval == 0:
                    ...
                    pbar.set_description(f"Fold {fold} | Iter {self.global_step}/{self.num_iterations} | Train Loss: {train_loss:.4f}, Acc: {train_acc_masked:.4f} | Val Loss: {val_loss:.4f}, Acc: {val_acc_masked:.4f}")
