...
class Trainer:
    ...
    def init_new_fold(self, fold):
        # run_name = f"{self.config.data.data_dir.split('/')[-1]}_{self.config.model.name}_{self.config.dataset.name}_Fold{fold}"
        # fold_wandb = wandb.init(project='Pansori_Sweep', name=run_name, dir=os.getcwd(), mode="online", reinit=True)
...
class FrameTrainer(Trainer):
    ...
    def train(self):
        ...
        for fold, (train_hash_keys, test_hash_keys) in enumerate(selection, start=1):
            ...
            # fold_wandb = self.init_new_fold(fold)
            ...
            while self.global_step < self.num_iterations:
                ...
                for batch in train_loader:
                    ...
                    # fold_wandb.log({"Train/Loss": loss, "Train/ACC": batch_acc, "Train/Masked Acc": batch_acc_masked}, step=self.global_step)
                    # fold_wandb.log({f"Fold{fold}/Train/Loss": loss, f"Fold{fold}/Train/ACC": batch_acc, f"Fold{fold}/Train/Masked Acc": batch_acc_masked}, step=self.global_step)
                    ...
                    if self.global_step % self.eval_interval == 0:
                        ...
                        wandb.log({f"Valid/Loss": val_loss, f"Valid/Acc": val_acc, f"Valid/Masked Acc": val_acc_masked, f"Valid Confusion Matrix": wandb.Image(Image.fromarray(cm))}, step=self.global_step)
                        # fold_wandb.log({f"Valid/Loss": val_loss, f"Valid/Acc": val_acc, f"Valid/Masked Acc": val_acc_masked, f"Valid Confusion Matrix": wandb.Image(Image.fromarray(cm))}, step=self.global_step)
                        # fold_wandb.log({f"Fold{fold}/Valid/Loss": val_loss, f"Fold{fold}/Valid/Acc": val_acc, f"Fold{fold}/Valid/Masked Acc": val_acc_masked, f"Fold{fold}/Valid Confusion Matrix": wandb.Image(Image.fromarray(cm))}, step=self.global_step)
                    ...
                    # Break
                    if self.global_step % self.num_iterations == 0: break
            ...
            wandb.log({f"Test/Loss": test_loss, f"Test/Acc": test_acc, f"Test/Masked Acc": test_acc_masked, f"Test Confusion Matrix": wandb.Image(Image.fromarray(cm))}, step=self.global_step)
            # fold_wandb.log({f"Test/Loss": test_loss, f"Test/Acc": test_acc, f"Test/Masked Acc": test_acc_masked, f"Test Confusion Matrix": wandb.Image(Image.fromarray(cm))}, step=self.global_step)
            # fold_wandb.log({f"Fold{fold}/Test/Loss": test_loss, f"Fold{fold}/Test/Acc": test_acc, f"Fold{fold}/Test/Masked Acc": test_acc_masked, f"Fold{fold}/Test Confusion Matrix": wandb.Image(Image.fromarray(cm))}, step=self.global_step)
...
class SegmentTrainer(Trainer):
    ...
    def train(self):
        ...
        for fold, (train_hash_keys, test_hash_keys) in enumerate(selection, start=1):
            ...
            # Start iter loop
            pbar = tqdm(total=self.num_iterations, desc=f"Fold {fold}")
            while self.global_step < (self.num_iterations * len(selection)):
                # Forward
                try: _, x, y = next(train_iter)
                except StopIteration: 
                    # update train iter
                ...
                loss, batch_acc, batch_acc_masked = self.train_batch(x, y)
                wandb.log({f"Train/Loss": loss, f"Train/ACC": batch_acc, f"Train/Masked Acc": batch_acc_masked}, step=self.global_step)
